...
pub struct CPUBus {
     ...
     * 前 256 字节划分为 Zero page，这块内存相比其他区域不同点在于能让 CPU 以更快的速度访问，所以需要频繁读写的数据会优先放入此区域，这块有单独的寻址模式
     * The first 256 bytes of adressable memory are called Zeropage. 
     * The 6502 processor family offers a wide selection of adressing modes to work with this part of the memory, 
     * which generally results in shorter and (even more important) faster code.
     * 
     * 0x0100 - 0x01FF ( Stack )
     * 这一块区域用于栈数据的存储，SP（栈指针） 从 0x1FF 处向下增长
     * Following the Zeropage, the next 256 bytes (located at $0100-$01FF) are used as processor stack. 
     * The stack function of this part is defined as it is in most other CPU's: 
     * Writing to stack will automatically decrement the stack pointer, 
     * while reading from it will increment it.
     ...
     * IO端口，包括 PPU，APU，输入设备的寄存器。比如 CPU 如果想读写 VRAM 的数据，就得靠 PPU 寄存器作为中介
     */
